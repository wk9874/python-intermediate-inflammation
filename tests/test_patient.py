...
def test_patients_added_correctly():
    ...
    assert doc.list_patients is not None
    assert len(doc.list_patients) == 1
...
def test_std_dev():
    ...
    mean = 5.0
    stddev = 3.0
    std_dev_array = numpy.random.normal(loc=mean, scale=stddev, size=100000)
    numpy.testing.assert_array_almost_equal(numpy.mean(std_dev_array), mean, decimal=2)
    numpy.testing.assert_array_almost_equal(numpy.std(std_dev_array), stddev, decimal=2)